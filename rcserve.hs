...
import System.IO (hGetContents, hPutStr, hSetBinaryMode, withFile, hClose, IOMode(..), Handle(..))
...
processRequest :: Handle -> IO String
processRequest h = do
    s <- hGetContents h
...
responseFile :: FilePath -> [Char] -> Handle -> IO ()        
...
processRequests :: Socket -> String -> IO b
